...
class PosOrder(models.Model):
    ...
    def load_order_line_details(self, line_id):
        data = {}
        line_obj = self.env['pos.order.line'].search_read(
            [('id', '=', line_id)])
        if line_obj:
            order_obj = self.browse(line_obj[0].get('order_id')[0])
            data['id'] = line_obj[0].get('id')
            data['product_id'] = line_obj[0].get('product_id')
            data['uom_id'] = self.env['product.product'].browse(
                line_obj[0].get('product_id')[0]).uom_id.name
            ...
            data['tabel_id'] = [order_obj.table_id.id,
                                order_obj.table_id.name] if order_obj.table_id else False
            data[
    ...
    def create_from_ui(self, orders):
        ...
        order_to_update = [o for o in orders if o['data']
        ...
        for tmp_order in orders_to_save:
            ...
            # raise UserError(_(str(pos_order.amount_total)+"-"+str(pos_order.amount_paid)))

            if (abs(pos_order.amount_total - pos_order.amount_paid) > 0.00001):
    ...
    def _process_order(self, order):
        ...
        draft_order_id = self.search(
            [('pos_reference', '=', submitted_references)]).id
        ...
        for order in pos_order:
            ...
            for line in order.lines:
                order_line = {
                    ...
                    'order_ref': line.order_id.pos_reference[16:20],
                    'mo': line.mo_id.id,
                }
...
class PosOrderLines(models.Model):
    ...
    @api.model
    def get_order_line_state(self, order_name, product_id, qty):
        order_id = self.env['pos.order'].search([('pos_reference', '=', order_name)]).id
        if order_id:
            order_line = self.search([('order_id', '=', order_id), ('product_id', '=', product_id), ('qty', '=', qty)])
            return order_line.state

    @api.model
    def get_order_line_mo(self, order_name, product_id, qty):
        order_id = self.env['pos.order'].search([('pos_reference', '=', order_name)]).id
        if order_id:
            order_line = self.search([('order_id', '=', order_id), ('product_id', '=', product_id), ('qty', '=', qty)])
            return order_line.mo_id.id
    ...
    def update_orderline_state(self, vals):
        ...
        _order = self.env['pos.order'].search([('id','=',order_line.order_id.id)])
        # if not _order.is_picked:
        #         #     _order.is_picked = _order.sudo().create_picking()
        ...
        if vals:
            for val in vals:
                ...
                if val.get('route'):
                    if val.get('state') == 'waiting':
                    ...
                    elif val.get('state') == 'check availability':
                        order_line = self.browse(val['order_line_id'])
                        if order_line.mo_id:
                            available = self.env['mrp.production'].pos_check_stock(order_line.mo_id.id)
                            if available:
                                state = 'available'
                            else:
                                state = 'not available'
                    elif val.get('state') == 'available':
                        state = 'preparing'
                        ...
                        state = 'cancel'
                    elif val.get('state') == 'not available':
                        order_line = self.browse(val['order_line_id'])
                        mo_canceled = self.env['mrp.production'].pos_production_cancel(order_line.mo_id.id)
                        if mo_canceled:
                            state = 'cancel'
                else:
                    if val.get('state') == 'waiting':
                ...
                if state:
                    ...
                    _order = self.env['pos.order'].search([('id','=',order_line.order_id.id)])
                    # if not _order.is_picked:
                    #     _order.is_picked = _order.create_picking()
    ...
    state = fields.Selection(
        selection=[("waiting", "Waiting"), ("check availability", "Check Availability"), ('available', 'Available'), ("preparing", "Preparing"), ("delivering", "Waiting/deliver"),
                   ("done", "Done"), ("cancel", "Cancelled"), ("not available", "Not Available")], default="waiting")
